 ...
 ******************************************************************************/
template<>
void buffer::append<char>(char c, Endian e)
{
...
template<>
void buffer::append<const char *>(const char *c, Endian e)
{
    (void) e; /* ignore endianess */
...
template<>
void buffer::append<const buffer &>(const buffer &b, Endian e)
{
    (void) e; /* ignore endianess */
...
void buffer::raw_append(const u8 *raw, unsigned size)
{
    ...
    {
        // std::cout << "Appending: " << raw[i] << " (" << (int) raw[i] << ")" << std::endl;
...
buffer buffer::escape()
{
    ...
    for (u8 byte : *this)
    {
        ...
        switch(byte)
        {
            ...
            case '\033': escaped.append('e'); break;
            default:
                escaped.append('x');
...
void buffer::grow(unsigned new_size)
{
    ...
    std::cout << "Growing buffer from " << _capacity << " to " << new_cap << std::endl;
