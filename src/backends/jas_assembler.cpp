...
bool JASAssembler::is_var(string name)
{
    return accessible_vars.count(name);
}

void JASAssembler::BIPUSH(int8_t value){ cs << "    BIPUSH " << (int) value << '\n'; }
...
void JASAssembler::LDC_W(string constant){ cs << "    LDC_W " << constant << '\n'; }
void JASAssembler::ILOAD(string var){ cs << "    ILOAD " << var << '\n'; }
void JASAssembler::IINC(string var, int8_t value){ cs << "    IINC " << var << ' ' << (int) value << '\n'; }
...
void JASAssembler::ICMPEQ(string label){ cs << "    ICMPEQ " << label << '\n'; }
