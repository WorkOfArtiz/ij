...
{
  // log.info("%s <- %u", sprint("%s#%s", current_func, name).c_str(), code.size());
  laddrs[sprint("%s#%s", current_func, name)] = code.size();
}

void IJVMAssembler::function(string name, vector<string> args, vector<string> vars)
{
...
bool IJVMAssembler::is_var(string name)
{
  return contains(vars, name);
}
...
void IJVMAssembler::DUP()
{
  code.append<u8>(op_dup);
}

void IJVMAssembler::IADD()
{
  code.append<u8>(op_iadd);
}

void IJVMAssembler::IAND()
{
  code.append<u8>(op_iand);
}

void IJVMAssembler::IOR()
{
  code.append<u8>(op_ior);
}

void IJVMAssembler::ISUB()
{
  code.append<u8>(op_isub);
}

void IJVMAssembler::POP()
{
  code.append<u8>(op_pop);
}

void IJVMAssembler::SWAP()
{
  code.append<u8>(op_swap);
}
...
void IJVMAssembler::WIDE()
{
  code.append<u8>(op_wide);
}

void IJVMAssembler::HALT()
{
  code.append<u8>(op_halt);
}

void IJVMAssembler::ERR()
{
  code.append<u8>(op_err);
}

void IJVMAssembler::IN()
{
  code.append<u8>(op_in);
}

void IJVMAssembler::OUT()
{
  code.append<u8>(op_out);
}

void IJVMAssembler::NOP()
{
  code.append<u8>(op_nop);
}
...
void IJVMAssembler::link(std::unordered_map<string, u32> findexes)
{
  ...
  for (std::pair<u32, string> p : jmpaddrs)
  {
    ...
    code.write(static_cast<i16>(address - p.first), p.first + 1, Endian::Big);
  }
  ...
  for (std::pair<u32, string> p : invokes)
  {
    if (findexes.count(p.second) == 0)
    {
      ...
      log.panic("INVOKEVIRTUAL call to unfound function '%s'", p.second.c_str());
    }
