...
template<class T>
inline int indexOf(std::vector<T> &v, const T &value)
{
...
static inline void sprint_helper(std::stringstream &out, const char *fmt, ArgType arg, ArgTypes ... args)
{
    for (const char *s = fmt; *s; s++)
    {
        if (*s == '%' && *(++s) != '%')
        {
            switch (*s)
            {
                case 'd': out << arg; break;
                case 's': out << arg; break;
                case 'x': out << std::hex << arg; break;
                default:
                    throw std::runtime_error{std::string("unsupported format: ") + *s};
            }
